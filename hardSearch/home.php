...
<section id="hero">
    ...
    <div class="search">
        ...
        <form action="" method ="GET">
            ...
            <select name="selection" style="font-size:20pt;margin-bottom:20px;">
                ...
                <option value="3" <?php if($selection==3){ echo "selected";}?>>Current BPL</option>
                ...
                <option value="6" <?php if($selection==6){ echo "selected";}?>>FatherName</option>
            </select>
    ...
    <div class="display_data">
        ...
        if(isset($_GET['household']) || isset($_GET['addid']) || isset($_GET['deleteid'])){
            ...
            if($selection==3){
                    $sql=" select * from $pd where pbl like '%$household%';";
            }
            if($selection==4){
                ...
                while($row=$result->fetch_assoc()){
                    ...
                    switch ($st){
                        case "NTN":
                            ...
                            break;
                        case "TN":
                            $table_background_string='background-color:pink;color:white;';break;
                        case "problem":
...
font-size: 16pt;background-color:white; ".$table_background_string."width:100%;height:100px;
 ...
 text-align: center; border-color:white'><tr>
                                  ...
                                  if($st!='problem' || $logininfo=='abdullah') {
                                      echo " <td>".$row['pbl']."</td>";
                                  }else{
                    ...
                    if($row['status']=="NTN"){
                        echo"<button style='font-size:16pt;' class='receivebutton' onclick=\"document.getElementById('household').disabled='false'\"><a href='home.php?addid=".$row['hh']." & addidpbl=".$row['pbl']."& household=".$row['hh']."'  style='text-decoration: none'>Received</a></button>";
                        echo "<script>document.getElementById('household').disabled=true</script>";
                    }
                    if($row['status']=="NTN"){
                        echo"<button style='font-size:16pt; float:right;' onclick=\"document.getElementById('household').disabled='false'\"><a href='home.php?notthisone=on' style='text-decoration: none;'>Not This One</a></button>";
                    }
                    else if($row['status']=="TN"){
                        echo "<button style='background-color: red;font-size:16pt; '><a style='color:white;' href='home.php?deleteidpbl=".$row['pbl']."& household=".$row['hh']."' style='text-decoration: none'>Delete</a></button>";
                        $_distribuation_state="done";
                    }
                    if($logininfo=='abdullah' && $row['status']=='problem') {
                        echo"<button style='font-size:16pt;' class='receivebutton' onclick=\"document.getElementById('household').disabled='false'\"><a href='home.php?addid=".$row['hh']." & addidpbl=".$row['pbl']."& household=".$row['hh']."'  style='text-decoration: none'>Received</a></button>";
                    }
                    if($logininfo=='abdullah'){
                        echo"<button style='font-size:16pt; align-self:center' class='receivebutton' onclick=\"document.getElementById('household').disabled='false'\"><a href='home.php?addid=".$row['hh']." & problem=".$row['pbl']."& household=".$row['hh']."'  style='text-decoration: none'>Problem</a></button>";
                ...
                $sql=" select * from conflict_december where hh like '%$household%';";
                $result=$con->query($sql);
            }
        }
        ...
        if(isset($_GET['problem'])) {
            ...
            $sql = "UPDATE $pd set status='problem' WHERE pbl=$addid";
            $con->query($sql);
        }
        ...
        if(isset($_GET['addidpbl'])){
            ...
            echo "<script>document.getElementById('household').disabled=false</script>";
            echo "<script>document.getElementById('household').autofocus=true</script>";
            $sql="UPDATE $pd set status='TN' WHERE pbl=$addid";
            $sql2="update $pd set username='$logininfo' WHERE pbl=$addid";
            $date=date("d-m-y");
            $sql3="update $pd set date='$date' WHERE pbl=$addid";
            $con->query($sql);
            $con->query($sql2);
            $con->query($sql3);
        }
        ...
        if(isset($_GET['deleteidpbl'])){
            ...
            $sql="UPDATE $pd set status='NTN' WHERE pbl=$deleteid";
            $sql2=" update $pd set username=' ' WHERE pbl=$deleteid";
            $sql3=" update $pd set date=' ' WHERE pbl=$deleteid";
            $con->query($sql);
            $con->query($sql2);
            $con->query($sql3);
...
<div class="total">
    <div class="first">
        <h1 align="center" style="font-size:40pt;color:black;">
            <?php
            $date=date('d-m-y');
            $sql="SELECT * FROM $pd where username='$logininfo' and date='$date'";
            $result=$con->query($sql);
